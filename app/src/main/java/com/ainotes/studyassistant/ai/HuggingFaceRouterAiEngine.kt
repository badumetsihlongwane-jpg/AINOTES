...
class HuggingFaceRouterAiEngine(
    ...
    override suspend fun generatePlan(input: StudyAiInput): AiStudyPlanResult = withContext(Dispatchers.IO) {
        if (!isConfigured) {
            return@withContext fallbackResult(
                summary = "AI autopilot is off. Add HF_TOKEN to enable auto-planning.",
                raw = null
            )
        }
        ...
        try {
            ...
            val generatedText = extractMessageContent(response.body)
...
""".trimIndent()
    ...
    private fun postJson(endpoint: String, token: String, jsonBody: String): HttpResponse {
    ...
    private fun extractMessageContent(responseBody: String): String {
        ...
        val contentAny = message.opt("content") ?: return ""

        if (contentAny is String) {
            return contentAny
        }

        if (contentAny is JSONArray) {
    ...
    private fun buildFailureSummary(statusCode: Int, model: String, errorMessage: String): String {
        ...
        return when (statusCode) {
            401, 403 -> {
                "HF Router auth failed (HTTP $statusCode). Check HF_TOKEN. $details"
            }
            404 -> {
                "HF model '$model' not found (HTTP 404). Check HF_MODEL provider suffix. $details"
            }
            else -> {
                "HF request failed with HTTP $statusCode on model '$model'. $details"
            }
        }
    }
    ...
    companion object {
        private const val DEFAULT_MODEL = "google/gemma-4-31B-it:novita"
    }
}
